<?php
    ...
    include_once("conexao/ProjetoVO.class.php");
  session_start();
  #Informações gerais
  ...
  $ID_Projeto             = $_SESSION["itemAtual"];
  $nome                   = $_POST['nome'];
  $autor_principal        = $_POST['autor_principal'];
  $email_autor_principal  = $_POST['email_autor_principal'];
  $nome_demais_autores    = $_POST['nome_demais_autores'];
  $email_demais_autores   = $_POST['email_demais_autores'];
  $link_repo              = $_POST['link_repo'];
  $palavra_chave          = $_POST['palavra_chave'];
  
  $link_video             = $_POST['link_video'];
  $link_site              = $_POST['link_site'];
  $tem_item               = $_SESSION['ID_Item'];
  ...
  $labelAlerta = '';
  $labelLink = '';
  $Link = 'TelaInserirProjeto.php';

  $Projeto = new ProjetoVO();
      
    $Projeto->ID_Projeto             =$ID_Projeto;
    $Projeto->nome                   =$nome;
    $Projeto->palavra_chave          =$palavra_chave   ;
    $Projeto->img_projeto            =$caminho_img_componente;
    $Projeto->link_repo              =$link_repo;
    $Projeto->link_site              =$link_site;
    $Projeto->metodologia            =$metodologia;
    $Projeto->tem_item               =$tem_item;
    $Projeto->link_video             = $link_video;
    #$Projeto->media_avaliacao        =$media_avaliacao;
    $Projeto->autor_principal       =$autor_principal;
    $Projeto->email_autor_principal =$email_autor_principal;
    $Projeto->nome_demais_autores   =$nome_demais_autores;
    $Projeto->email_demais_autores  =$email_demais_autores ;
   
  
  $fachada = new Fachada();

  $result = $fachada->atualizarProjeto($Projeto);
  #echo "Resultado: ".$result;
  #echo $result."Tá retornando isso!";
  if ($result == -1) {
    $imprimir = "Projeto não cadastrado!";
    $tipoAlert = "alert alert-warning alert-dismissible fade show";
    $labelAlerta = 'Atenção! ';
    $labelLink = 'Tente cadastrar um Projeto novo';
  }else if ($result == -2) {

    $imprimir = "Erro ao atualizar Projeto!";
    $tipoAlert = "alert alert-danger";
    $labelAlerta = 'Erro! ';
    $labelLink = 'Tente novamente';
  }else
  {
    $imprimir = " Projeto atualizado com sucesso!";
    $tipoAlert = "alert alert-success";
    $labelAlerta = 'Sucesso! ';
    $labelLink = 'Visualizar!';
    $Link = 'TelaExibirProjeto.php';
    $_SESSION["ProjetoAtual"] = $result;
