...
include_once("conexao/SensorVO.class.php");
...
if(isset($_FILES['caminho_img_componente'])) { 
  ...
  $idItem                = $_SESSION["itemAtual"];
	...
	$labelAlerta = '';
	$labelLink = '';
	$Link = 'TelaInserirSensor.php';

	$Sensor = new SensorVO();

  $Sensor->ID_Item             = $idItem;
	$Sensor->img_componente 	   = $caminho_img_componente;
	$Sensor->nome 			         = $nome;
	$Sensor->modelo 				     = $modelo;			
	$Sensor->temperaturaOperacao = $temperatura_ope;
	$Sensor->dimensao 			     = $dimensao;
	$Sensor->linkDS				       = $link_DS;
	$Sensor->precoMedio 			   = $precoMedio;
	$Sensor->palavra_chave       = $palavra_chave;	
	
	$Sensor->tensaoSaida        = $tensaoSaida;
	$Sensor->funcao 			    	= $funcao ;
	$Sensor->compativel         = $compativel;
	$Sensor->tensaoOperacao	    = $tensaoOperacao;	
	$Sensor->infoGerais			    = $infoGerais;
	 
	
	$fachada = new Fachada();

	$result = $fachada->atualizarSensor($Sensor);
