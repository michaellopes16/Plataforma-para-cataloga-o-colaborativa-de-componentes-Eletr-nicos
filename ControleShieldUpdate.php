...
<?php
    ...
    include_once("conexao/ShieldVO.class.php");
 	session_start();
 	#Informações gerais
#caminho_img_componente
...
if(isset($_FILES['caminho_img_componente'])) { 
  ...
  $compativel            = $_SESSION['ID_Item'];
	...
	$labelAlerta = '';
	$labelLink = '';
	$Link = 'TelaInserirShield.php';

	$Shield = new ShieldVO();
   $Shield->ID_Item               = $idItem; 
	 $Shield->img_componente 	      = $caminho_img_componente;
	 $Shield->nome 			            = $nome;
	 $Shield->modelo 				        = $modelo;			
	 $Shield->temperaturaOperacao   = $temperatura_ope;
	 $Shield->dimensao 			        = $dimensao;
	 $Shield->linkDS				        = $link_DS;
	 $Shield->precoMedio 			      = $precoMedio;
	 $Shield->palavra_chave         = $palavra_chave;	
	
	 $Shield->peso         		      = $peso;
	 $Shield->funcao 				        = $funcao ;
	 $Shield->compativel            = $compativel;
	 $Shield->tensaoOperacao	      = $tensaoOperacao;
	 $Shield->modo_consumo			    = $modo_consumo;	
	 $Shield->infoGerais			      = $infoGerais;
	 
	
	$fachada = new Fachada();

	$result = $fachada->atualizarShield($Shield);
